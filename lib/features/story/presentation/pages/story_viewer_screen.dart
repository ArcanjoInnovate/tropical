...
class _StoryViewerScreenState extends State<StoryViewerScreen>
  ...
  StoryModel? get _currentStory => _storyIndex < _currentStories.length
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  VÍDEO ATIVO - CORREÇÃO DE ÁUDIO PERSISTENTE
  // ══════════════════════════════════════════════════════════════════════════

  /// Para completamente o vídeo atual (pausa + reseta) antes de trocar.
  void _pauseAndStopCurrentVideo() {
    if (_videoCtrl == null) return;

    try {
      // Remove o listener para evitar callbacks durante a transição
      _videoCtrl!.removeListener(_onVideoTick);

      // Pausa o vídeo
      _videoCtrl!.pause();

      // Reseta para o início (evita áudio residual)
      _videoCtrl!.seekTo(Duration.zero);

      debugPrint('[StoryViewer] 🔇 Vídeo pausado e resetado');
    } catch (e) {
      debugPrint('[StoryViewer] ⚠️ Erro ao parar vídeo: $e');
    }
  }
  ...
  Future<void> _initVideo(String url) async {
    ...
    // 🔧 FIX: Para completamente o vídeo anterior antes de trocar
    _pauseAndStopCurrentVideo();
  ...
  Future<void> _startStory() async {
    ...
    _progressCtrl.stop();
    _progressCtrl.removeStatusListener(_onProgressStatus);
    _progressCtrl.reset(); // ← zera value para 0.0
    ...
    if (story.isVideo && story.mediaUrl != null) {
      ...
      if (!_paused) {
        _progressCtrl.forward(from: 0.0); // ← from: 0.0 é obrigatório
      }
  ...
  Future<void> _deletarStory() async {
    ...
    try {
      ...
      if (cachedCtrl != null) {
        cachedCtrl.pause(); // 🔧 Para antes de descartar
        cachedCtrl.dispose();
      }
  ...
  Future<void> _abrirDenuncia() async {
    ...
    _pauseStory();
    await ReportPage.push(
      context,
      config: ReportPageConfig.story(
        storyId: story.id,
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        decoration: const BoxDecoration(
            color: Color(0xFF130020),
            border:
                Border(top: BorderSide(color: Color(0x33FF2D7A), width: 0.8))),
        child: SafeArea(
            top: false,
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Center(
...
class _StoryBackground extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
        ...
        if (story.thumbUrl != null)
          AnimatedOpacity(
            ...
            child: CachedNetworkImage(
              imageUrl: CloudinaryHelper.storyUrl(story.thumbUrl!),
              fit: BoxFit.cover,
    ...
    if (story.type == 'camera' && story.mediaUrl != null) {
      return Stack(fit: StackFit.expand, children: [
        CachedNetworkImage(
            imageUrl: CloudinaryHelper.storyUrl(story.mediaUrl!),
            fit: BoxFit.cover,
            fadeInDuration: const Duration(milliseconds: 200),
    ...
    return [
      Positioned.fill(
        child: LayoutBuilder(builder: (_, constraints) {
          ...
          return Stack(
              children: story.overlays.map((overlay) {
  ...
  TextStyle _overlayTextStyle(String? fontStyle, double scale) {
    ...
    return base
        .copyWith(fontSize: (base.fontSize ?? 26) * scale, shadows: const [
