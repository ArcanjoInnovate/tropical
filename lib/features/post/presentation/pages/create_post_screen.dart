...
//   • Método _publicar: uploads de foto, vídeo e thumbnail reescritos
//     para CloudinaryService (igual ao padrão de create_gallery_screen
//     e create_story_screen).
...
class _CriarPostScreenState extends State<CreatePostScreen>
  ...
  Future<void> _pickFoto(ImageSource src) async {
    ...
    final p = await _picker.pickImage(
        source: src, maxWidth: 1080, maxHeight: 1080, imageQuality: 85);
    if (p == null) return;
  ...
  Future<void> _carregarVideo(File file) async {
    ...
    setState(() {
      ...
      _videoProcessStep    = 'Preparando vídeo...';
    });

    File videoProcessado;
    try {
      _atualizarProgressoProcessamento(mySession, 0.1, 'Analisando vídeo...');
      await Future.delayed(const Duration(milliseconds: 300));

      _atualizarProgressoProcessamento(mySession, 0.3, 'Cortando vídeo...');
      videoProcessado = await VideoTrimService.trimAndCompress(
        file: file,
        maxSeconds: _maxVideoSeconds,
      );

      if (!mounted || _videoSessionId != mySession) {
        try { videoProcessado.deleteSync(); } catch (_) {}
        return;
      }

      _atualizarProgressoProcessamento(mySession, 0.8, 'Finalizando...');
      await Future.delayed(const Duration(milliseconds: 200));
    } catch (e) {
      debugPrint('[Carregar] $e');
      if (!mounted || _videoSessionId != mySession) return;
      setState(() {
        _publishStep      = _PublishStep.erro;
        _videoProcessStep = 'Erro ao processar. Usando original.';
      });
      await Future.delayed(const Duration(milliseconds: 500));
      videoProcessado = file;
    }

    if (!mounted || _videoSessionId != mySession) return;

    _atualizarProgressoProcessamento(mySession, 0.9, 'Carregando preview...');

    final playerCtrl = VideoPlayerController.file(videoProcessado);
    ...
    setState(() {
      _video               = videoProcessado;
      _videoCtrl           = playerCtrl;
      _videoDuration       = playerCtrl.value.duration;
      _videoTamanhoOriginal =
          '${(videoProcessado.lengthSync() / (1024 * 1024)).toStringAsFixed(1)} MB';
      _publishStep          = _PublishStep.idle;
      _videoProcessProgress = 1.0;
    });

    _gerarThumbnailPrimeiroFrame(videoProcessado, mySession);
  }

  void _atualizarProgressoProcessamento(
      int session, double progress, String step) {
    if (!mounted || _videoSessionId != session) return;
    setState(() {
  ...
  Future<void> _comprimirVideoBackground(
      File original, int mySession) async {
  ...
  Future<void> _gerarThumbnailPrimeiroFrame(
    ...
    try {
      ...
      final cmd =
          '-y -i "${videoFile.path}" -vframes 1 -q:v 2 "$thumbPath"';
      final session = await FFmpegKit.execute(cmd);
      final rc      = await session.getReturnCode();
      if (!ReturnCode.isSuccess(rc)) return;

      final thumbFile = File(thumbPath);
      if (!thumbFile.existsSync()) return;
      final bytes = await thumbFile.readAsBytes();
      thumbFile.deleteSync();

      if (!mounted || _videoSessionId != mySession || _capaPersonalizada) {
        return;
      }
  ...
  void _abrirSeletorCapa() {
    if (_video == null ||
        _videoCtrl == null ||
        !_videoCtrl!.value.isInitialized) return;
    _videoCtrl!.pause();
    HapticFeedback.selectionClick();
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  PUBLICAR — Firebase Storage → Cloudinary
  // ══════════════════════════════════════════════════════════════════════════
  Future<void> _publicar() async {
    ...
    try {
      ...
      if (_tipoAtual == 'foto' && _foto != null) {
        ...
        mediaUrl = await CloudinaryService.instance.uploadBytes(
          bytes: watermarkedBytes,
          filename: 'wm_post_$ts.jpg',
        ...
        // Snapshot dos bytes da thumbnail antes de qualquer await longo
        ...
        final File videoFinal = watermarkedVideo ?? _video!;
        ...
        if (thumbBytesSnapshot != null && thumbBytesSnapshot.isNotEmpty) {
          ...
          thumbUrl = await CloudinaryService.instance.uploadBytes(
            bytes: thumbWatermarked,
            filename: 'wm_thumb_$ts.jpg',
            resourceType: CloudinaryResourceType.image,
            folder: 'posts/$uid/thumbs',
            publicId: '$ts',
            onProgress: (p) {
              if (mounted) setState(() => _uploadProgress = p);
            },
      ...
      await PostService.instance.createPost(
        ...
        tipo: _tipoAtual,
        visibilidade: _visibilidade.name,
        mediaUrl: mediaUrl,
        thumbUrl: thumbUrl,
  ...
  Widget _buildVideoProcessOverlay() {
    final isError = _publishStep == _PublishStep.erro;
              ...
              if (!isError)
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: const Color(0xFFE85D5D), width: 2),
                  ),
              ...
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Flexible(
                    child: Text(
                      isError
                          ? 'Usando vídeo original sem otimização'
  ...
  Widget _buildProgressOverlay() {
    final label = switch (_publishStep) {
      ...
      _PublishStep.uploadingMedia  =>
        _tipoAtual == 'video' ? 'ENVIANDO VÍDEO...' : 'ENVIANDO FOTO...',
        ...
        Container(
            ...
            child: ClipRRect(
                ...
                child: avatarUrl.isNotEmpty
                    ? CachedNetworkImage(imageUrl: CloudinaryHelper.avatarUrl(avatarUrl),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
            ...
            child: TabBar(
                ...
                tabs: const [
                  Tab(
                      icon: Icon(Icons.photo_outlined, size: 15),
                      text: 'FOTO'),
                  Tab(
                      icon: Icon(Icons.text_fields_rounded, size: 15),
                      text: 'TEXTO'),
                  Tab(
                      icon: Icon(Icons.emoji_emotions_outlined, size: 15),
                      text: 'EMOJI'),
                  Tab(
  ...
  Widget _buildTabContent() => Padding(
      ...
      child: AnimatedSwitcher(
          ...
          child: switch (_tabController.index) {
            0 => KeyedSubtree(
                key: const ValueKey('foto'), child: _buildTabFoto()),
            1 => KeyedSubtree(
                key: const ValueKey('texto'), child: _buildTabTexto()),
            2 => KeyedSubtree(
                key: const ValueKey('emoji'), child: _buildTabEmoji()),
            3 => KeyedSubtree(
                key: const ValueKey('video'), child: _buildTabVideo()),
  ...
  Widget _buildTabFoto() =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _buildCampoDescricao(
            hint: 'Adicione uma legenda...', opcional: true),
      ]);

  Widget _buildTabTexto() => _buildCampoDescricao(
      hint: 'Escreva o que está rolando...', opcional: false);

  Widget _buildTabEmoji() =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        AnimatedContainer(
            duration: const Duration(milliseconds: 300),
            width: double.infinity,
        ...
        ..._emojiGroups.entries.map((group) =>
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              GridView.builder(
                  ...
                  itemBuilder: (_, i) {
                    final emoji = group.value[i];
                    final sel   = _emojiSelecionado == emoji;
                    return GestureDetector(
                        onTap: () {
                          setState(() =>
                              _emojiSelecionado = sel ? null : emoji);
                          HapticFeedback.selectionClick();
                        },
                        child: AnimatedContainer(
        ...
        _buildCampoDescricao(
            hint: 'Adicione uma legenda ao emoji...', opcional: true),
      ]);

  Widget _buildTabVideo() =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        GestureDetector(
            onTap:
                _video == null ? _showVideoSheet : _toggleVideoPlay,
            child: AnimatedContainer(
                duration: const Duration(milliseconds: 300),
                width: double.infinity,
        ...
        const SizedBox(height: 16),
        _buildCampoDescricao(
            hint: 'Adicione uma legenda ao vídeo...', opcional: true),
      ]);
  ...
  Widget _buildSizeChip() {
    final original   = _videoTamanhoOriginal ?? '?';
    final comprimido = _videoTamanhoComprimido;
    return Container(
        padding:
  ...
  Widget _buildDuracaoBadge() {
    ...
    final secs = _videoDuration!.inSeconds;
    final ok   = secs <= _maxVideoSeconds;
    return Container(
        padding:
  ...
  Widget _buildCampoDescricao(
          {required String hint, required bool opcional}) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          _Label('DESCRIÇÃO'),
          if (opcional) ...[
  ...
  Widget _buildVisibilidade() =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: _Visibilidade.values.map((v) {
          final sel    = _visibilidade == v;
          final isLast = v == _Visibilidade.values.last;
          return Expanded(
              child: GestureDetector(
                  onTap: () => setState(() => _visibilidade = v),
                  child: AnimatedContainer(
...
class _FramePickerSheetState extends State<_FramePickerSheet> {
  ...
  Future<void> _confirmar() async {
    ...
    try {
      await widget.videoCtrl
          .seekTo(Duration(milliseconds: _positionMs));
      widget.videoCtrl.pause();
      ...
      final dpr = ui.PlatformDispatcher.instance.implicitView
              ?.devicePixelRatio ??
          2.0;
      final image =
...
// ══════════════════════════════════════════════════════════════════════════════
//  BARRA DE PROGRESSO DO VÍDEO  (inalterada)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  ENUMS E WIDGETS AUXILIARES  (inalterados)
// ══════════════════════════════════════════════════════════════════════════════
enum _Visibilidade {
