...
class HomeScreenAdministrative extends StatefulWidget {
  ...
  @override
  State<HomeScreenAdministrative> createState() =>
      _HomeScreenAdministrativeState();
}

class _HomeScreenAdministrativeState extends State<HomeScreenAdministrative> {
  // ── Lista bruta, nunca filtrada ─────────────────────────────────────────
  List<PartyModel> _allFestas = [];

  // ── Getter que filtra reativamente ──────────────────────────────────────
  List<PartyModel> get _visibleFestas {
    if (_lastKnownBlockedIds.isEmpty) return _allFestas;
    return _allFestas
        .where((f) => !_lastKnownBlockedIds.contains(f.creatorId))
        .toList();
  }
  ...
  void initState() {
    super.initState();
    // ── Registra listener no BlockProvider após o primeiro frame ───────────
  ...
  @override
  void dispose() {
    // ── Remove o listener com segurança ─────────────────────────────────────
    try {
  ...
  void _onBlockProviderChanged() {
    ...
    if (changed) {
      ...
      // ── Força rebuild para aplicar novo filtro ────────────────────────────
      if (mounted) setState(() {});
    }
  }
  ...
  Future<void> _init() async {
    ...
    // ── Salva bruto — filtro fica no getter _visibleFestas ─────────────────
    setState(() {
      _homeCoords = results[0] as ({double latitude, double longitude})?;
      _allFestas = results[1] as List<PartyModel>;
      _loading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    // ← REATIVIDADE: escuta mudanças de bloqueio em tempo real
          ...
          SafeArea(
            ...
            child: CustomScrollView(
                ...
                slivers: [
                  ...
                  else if (_visibleFestas.isEmpty)
                    SliverFillRemaining(
                        hasScrollBody: false,
                        child: Center(
                            child: Column(
                    ...
                    SliverPadding(
                        ...
                        sliver: SliverList(
                            delegate: SliverChildBuilderDelegate(
                                (_, i) => _FestaListTile(
                                    festa: _visibleFestas[i],
                                    homeCoords: _homeCoords,
                                    onTap: () =>
                                        _abrirDetalhe(_visibleFestas[i])),
                                childCount: _visibleFestas.length))),
                ]),
          )),
...
class _FestaListTile extends StatelessWidget {
  ...
  String? get _distLabel {
    ...
    final km = LocationService.distanceKm(homeCoords!.latitude,
        homeCoords!.longitude, festa.latitude!, festa.longitude!);
        ...
        child: Container(
            ...
            child: Row(children: [
              SizedBox(
                  ...
                  height: 120,
                  child: temBanner
                      ? CachedNetworkImage(imageUrl: CloudinaryHelper.bannerUrl(festa.bannerUrl!),
                ...
                child: Padding(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Row(children: [
                            _Chip(
                                Icons.star_outline_rounded, festa.interessados),
                            const SizedBox(width: 10),
                        ...
                        GestureDetector(
                          onTap: () {
                            Navigator.push(
                              ...
                              PageRouteBuilder(
                                pageBuilder: (_, animation, __) =>
                                    PartyAttendeesScreen(festa: festa),
                                transitionsBuilder:
                    ...
                    child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color:
                                      const Color(0xFFE85D5D).withOpacity(0.4),
                                  width: 0.8)),
                          child: Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: const [
