...
class _FullscreenVideoScreenState extends State<FullscreenVideoScreen>
  ...
  void initState() {
    ...
    // Sincroniza mute com o estado atual do controller/global
    final cached = VideoPreloadService.instance.getController(widget.postId);
    if (cached != null) {
      _muted = cached.value.volume == 0.0;
    }
...
void dispose() {
  ...
  if (_fromCache) {
    ...
    _ctrl?.dispose(); // só descarta o controller do fullscreen
  }
  ...
  Future<void> _initController() async {
    ...
    // Pausa o controller inline para evitar dois players tocando ao mesmo tempo
    final cached = VideoPreloadService.instance.getController(widget.postId);
    cached?.pause();
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onHorizontalDragUpdate: (details) {
        ...
        setState(() =>
            _dragValue = (localX / box.size.width * total).clamp(0.0, total));
      },
